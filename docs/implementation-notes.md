...
Messages opens each sample conversation at `/messages/:postId`. The inbox and conversation check the active preview account; opening an available conversation marks its question read. Drafts live alongside the question in preview state, so internal navigation and account switching preserve unfinished replies. Enter inserts a line break; Cmd/Ctrl+Enter opens the review. Acceptance appends the reviewed paragraph to the post, clears the pending question and draft, and retains the question and accepted answer in the inbox and history. The reducer rejects stale or duplicate acceptance and writes by other accounts. Removing a post removes its conversation.

Conversation pages use the existing TanStack Router, TanStack Form and Effect validation, with no added dependencies or polling. The reply field grows to 200px before scrolling. Review and completion move keyboard focus to the new content; Edit reply returns it to the composer. The page uses normal document scrolling and dynamic viewport height, with room for the preview controls. Drafts, read state and history still reset on reload. Persistent multi-message conversations, model replies and server authorization remain future work. Old `/saved` and `/dashboard` links redirect to the corresponding feed filters.
