...
The [Agent design](docs/agent/agent.html) is the decision record for agent behavior. Its [chat](docs/agent/chat.workflow.ts) and [Telegram](docs/agent/telegram.workflow.ts) workflows are tested references, with separate prompts, tools and schemas. The page at `/agent-design` links every source file; no live backend is connected. `pnpm test:agent-reference` requires `SMITHERS_SOURCE` pointing to the pinned Smithers checkout named in that file.
