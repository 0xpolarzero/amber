...
The [Agent design](docs/agent/agent.html) is the decision record for agent behavior. Its [chat](docs/agent/chat.workflow.ts) and [Telegram](docs/agent/telegram.workflow.ts) workflows are tested references, with separate prompts, tools and schemas. The page at `/agent-design` records the decisions; no live backend is connected. `pnpm test:agent-reference` requires `SMITHERS_SOURCE` pointing to the pinned Smithers checkout named in that file.

Start with the [Telegram example test](docs/agent/telegram.test.ts): [fake messages and model responses](docs/agent/testing/telegram-fixtures.ts) go through the real Smithers workflow to produce [this saved result](docs/agent/telegram.result.json). Run it with `SMITHERS_SOURCE=/path/to/pinned/smithers pnpm test:telegram`. Model responses are scripted; this verifies the pipeline, not live model judgment.
